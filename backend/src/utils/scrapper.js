...
export async function scrapeStarTech(product) {
    ...
    // console.log("Scrapping startech", $(".p-item"));
...
export async function scrapeRyans(product) {
    ...
    $(".category-single-product").each((_, element) => {
        ...
        const price =
            $(element).find(".pr-text").text().trim() || "Out Of Stock";
        const img =
...
export async function scrapeUltraTech(product) {
    const url = `https://www.ultratech.com.bd/index.php?route=product/search&search=${encodeURIComponent(
        product
    )}`;
    const $ = await fetchHtml(url);
    const products = [];
...
export async function scrapeSkyLand(product) {
    const url = `https://www.skyland.com.bd/index.php?route=product/search&search=${encodeURIComponent(
        product
    )}`;
    const $ = await fetchHtml(url);
    const products = [];
